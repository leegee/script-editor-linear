...
export default function TimelineEditor() {
    ...
    async function processCompletedLine(div: HTMLElement, lineIndex: number) {
        const id = div.dataset.id;
        if (!id) throw new TypeError("DIV lacks dataset.id");

        const lines = div.innerText.split("\n");

        const completedLine = lines[lineIndex];
        const classification = classifyLine(completedLine.trim());
        ...
        // 2. Update current item with only BEFORE text
        div.innerText = before;
        await saveLine(div);

        // 3. Prepare new item's initial text
        let newText = "";

        console.log('classification.type', classification.type)

        if (classification.type === "dialogue") {
            // completedLine == character name, so text starts AFTER the name
            newText = afterText || "";
        } else {
            // completedLine contains: TYPE Title?
            // Extract everything after the keyword (the title)
            const parts = completedLine.trim().split(/\s+/);
            const title = parts.slice(1).join(" ");
            newText = title + (afterText ? "\n" + afterText : "");
        }

        // 4. Create new item and render
        let newId: string;
        if (classification.type === "dialogue") {
            newId = await createTimelineItemAfter(
                id,
                classification.type,
                { ref: classification.character, text: newText }
            );
        }
        else {
            const raw = completedLine;
